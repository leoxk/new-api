...
func checkAndSendQuotaNotify(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int) {
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			title, content := quotaNotificationMessage(
				userSetting.Language, notifyType, false, logger.FormatQuota(relayInfo.UserQuota), topUpLink,
			)
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	gopool.Go(func() {
		...
		title, content := quotaNotificationMessage(
			userSetting.Language, notifyType, true, logger.FormatQuota(int(remaining)), topUpLink,
		)
...
func quotaNotificationMessage(lang, notifyType string, subscription bool, remaining, topUpLink string) (string, string) {
	titleKey := i18n.MsgEmailQuotaLowSubject
	if subscription {
		titleKey = i18n.MsgEmailSubscriptionQuotaLowSubject
	}
	title := i18n.Translate(lang, titleKey)
	data := map[string]any{"Remaining": remaining, "Link": topUpLink}
	if notifyType == dto.NotifyTypeBark || notifyType == dto.NotifyTypeGotify {
		return title, i18n.Translate(lang, i18n.MsgEmailQuotaLowText, data)
	}
	return title, i18n.Translate(lang, i18n.MsgEmailQuotaLowBody, data)
}
